...
async def reg(session,code,rno):	
	...
	async with session.get(api,params=param,timeout=10) as resp:
            print(resp.status)
...
async def rcrv(session,code,rno):
	...
	async with session.get(api,params=param,timeout=9) as resp:
            res_status=resp.status
            re= await resp.text()
	return re ,res_status
...
async def main1(ap,rno):
    async with aiohttp.ClientSession(timeout=600) as session:
        ...
        async with asyncio.TaskGroup() as t:
        	for i in ap:
        		print(i)
        		results.append(t.create_task(fetch(session,i,rno)))
...
def index(rno):
    ...
    #mem rexognition
        ...
        print(r[0],rno)
        details,sts=asyncio.run(info(r[0],rno))
        print("ooooooooo")
